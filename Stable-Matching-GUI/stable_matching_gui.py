...
def validate_input(n, grp1, grp2, pref1, pref2):
    if (len(grp1) != n):
        return ([False, 'No of Males not Equal to n !!!'])
    elif (len(grp2) != n):
        return ([False, 'No of Females not Equal to n !!!'])
    elif (len(grp1) != len(grp2)):
        return ([False, 'No of Participants are unequal !!!'])
...
root = tk.Tk()
root.title("GALE-SHAPLEY ALGORITHM")
root.geometry("500x700")  # 300x200
lbs = tk.Label(root, text="STABLE MATCHING PROBLEM", font=20)
lbs.place(x=100, y=5)

countlabel = tk.Label(root, text="No.of Nodes ")
countEntry = tk.Entry(root)
countlabel.place(x=50, y=60)
countEntry.place(x=210, y=60)

grp1label = tk.Label(root, text="Male Candidates")
grp1Entry = tk.Entry(root)
grp1label.place(x=50, y=100)
grp1Entry.place(x=210, y=100, width=250)

grp2label = tk.Label(root, text="Female Candidates")
grp2Entry = tk.Entry(root)
grp2label.place(x=50, y=140)
grp2Entry.place(x=210, y=140, width=250)

pref1label = tk.Label(root, text="Pref. of Male Candidates ")
pref1text = tk.Text(root)
pref1label.place(x=50, y=180)
pref1text.place(x=210, y=180, height=100, width=250)

pref2label = tk.Label(root, text="Pref. of Female Candidates ")
pref2text = tk.Text(root)
pref2label.place(x=40, y=310)
pref2text.place(x=210, y=310, height=100, width=250)

log = tk.Button(root, font=("times", 16), text="Match",
                command=lambda: getInputs(countEntry.get(), grp1Entry.get(), grp2Entry.get(),
                                          pref1text.get("1.0", 'end-1c'), pref2text.get("1.0", 'end-1c'), root))
log.place(x=200, y=440)

inst_text = 'Note:-\n\tn = Integer\n\tM/F Candidates = comma separated\n\tPreferences = Comma Separated matrix'
